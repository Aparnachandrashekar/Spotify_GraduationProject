...
const TRACK_CACHE_TTL_MS = 60 * 60_000;
...
const trackByIdCache = new Map<string, { track: Track | null; expiresAt: number }>();
...
async function spotifyFetch(
  ...
  const endpoint = url.includes("/search")
    ? "search"
    : url.includes("/tracks/")
      ? "track"
      : "api";
...
export async function getTrackById(
  ...
  const resolvedMarket = market ?? getMarket();
  const cacheKey = `${resolvedMarket}:${id}`;
  const cached = trackByIdCache.get(cacheKey);

  if (cached && Date.now() < cached.expiresAt) {
    console.info(`[spotify] cache hit track ${id}`);
    return cached.track;
  }

  const params = withMarket(new URLSearchParams(), resolvedMarket);
  const response = await spotifyFetch(`${TRACK_URL}/${id}?${params.toString()}`);

  if (response.status === 404) {
    trackByIdCache.set(cacheKey, {
      track: null,
...
export async function lookupTrack(
  ...
  const simpleQuery = `${title} ${primaryArtist}`;
  const structuredQuery = `track:"${title}" artist:"${primaryArtist}"`;

  const tryQuery = async (
    query: string,
    markets: string[],
  ): Promise<Track | null> => {
    const tracks = await cachedMarketSearch(query, 8, markets);
    return pickBestMatch(tracks, title, artist);
  };

  try {
    let match = await tryQuery(simpleQuery, primaryMarket);
    ...
    if (!match && shouldUseIndianMarkets(title, artist)) {
      ...
      if (!match) {
        match = await tryQuery(structuredQuery, ["IN", "US"]);
      }
    }
